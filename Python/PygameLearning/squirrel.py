...
def main():
    ...
    L_SQUIR_IMG = pygame.image.load('images/squirrel.png')
...
def runGame():
    ...
    # start off with some random grass images on the scree
    for i in range(10):
    ...
    while True: # main game loop
        ...
        playCenterx = playerObj['x'] + int(playerObj['size'] / 2)
        playCentery = playerObj['y'] + int(playerObj['size'] / 2)
        if ( camerax + HALF_WINWIDTH) - playCenterx > CAMERASLACK:
            camerax = playCenterx + CAMERASLACK - HALF_WINWIDTH
        elif playCenterx - (camerax + HALF_WINWIDTH) > CAMERASLACK:
            camerax = playCenterx - CAMERASLACK - HALF_WINWIDTH
        if (cameray + HALF_WINHEIGHT) - playCentery > CAMERASLACK:
            cameray = playCentery + CAMERASLACK - HALF_WINHEIGHT
        elif playCentery - (cameray + HALF_WINHEIGHT) > CAMERASLACK:
            cameray = playCentery - CAMERASLACK - HALF_WINHEIGHT
        ...
        if not gameOverMode:
            ...
            for i in range(len(squirrelObjs)-1, -1, -1):
                ...
                if 'rect' in sqObj and playerObj['rect'].colliderect(sqObj['rect']):
                    # player/squirrel collision has occurred
                    ...
                    if sqObj['width'] * sqObj['height'] <= playerObj['size']**2:
                        ...
                        if playerObj['health'] == 0:
                            gameOverMode = True # true on "game over mode"
...
def getRandomOffCameraPos(camerax, cameray, objWidth, objHeight):
    ...
    while True:
        ...
        # create a Rect object with the random corrdinates and use colliderect()
